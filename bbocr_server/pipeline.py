...
ONNX_PATH = getenv('ONNX_PATH', '/home/zihad/.bengali_ai_ocr/bnocr.onnx')
PADDLE_LOC = getenv('PADDLE_MODEL_PATH', '/home/zihad/.paddleocr')
image_dir = 'tests'
model_weight = getenv('MODEL_WEIGHT', '/home/zihad/.bengali_ai_ocr/best.pt')
base_path='./save'
preproc_suffix = 'v2'
...
pred_text_suffix = f'_paragraph_wise_recognized_text.pkl'
paragraph_wise_predicted_word_box_suffix = f'_paragraph_wise_word_boxes.pkl'
roi_suffix = f'_region_of_interests.pkl'
word_gc_all_box_all_lines_suffix = f'_word_gc_all_box_all_lines.pkl'
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=f'{PADDLE_LOC}/whl/det/en/en_PP-OCRv3_det_infer')
    parser.add_argument(
        "--det_model2", required=False, help="Path of Detection model of PPOCR.",
        default=f'{PADDLE_LOC}/whl/det/ml/Multilingual_PP-OCRv3_det_infer')
    
    parser.add_argument(
        "--rec_model", required=False, help="Path of Detection model of PPOCR.",
        default=f'{PADDLE_LOC}/whl/rec/en/en_PP-OCRv3_rec_infer')
    # parser.add_argument(
...
from apsisocr import DenseNet121BnEnClassifier, SVTRLCNet, ApsisNet

# Initialize models
lang_classifier = DenseNet121BnEnClassifier()
enocr = SVTRLCNet()
bnocr = ApsisNet()


def recognize_word(crops):
    #results = rec(crops=crops)
    print(f"Number of crops to recognize: {len(crops)}")  # Debug: Log number of crops

    texts = []
    for crop in crops:
        #print(f"Processing crop of shape: {crop.shape}")  # Debug: Log crop shape
        lang = lang_classifier.infer([crop])
        #print(f"Detected language: {lang}")  # Debug: Log detected language
        if lang == ['bn']:
            res = bnocr.infer([crop], )
            #print(f"Bangla OCR result: {res}")  # Debug: Log Bangla OCR result
        else:
            res = enocr.infer([crop], )
            #print(f"English OCR result: {res}")  # Debug: Log English OCR result
        texts.append(res[0])
    return texts
...
class HtmlGenerator:
    ...
    def create_html_file(self):
        ...
        with open(f"./save/html/{self.filename}.html", "w") as f:
...
def reconstruct(directory, img_src_save_dir, standalone_OCR_required):
    ...
    for idx, file_name in tqdm(enumerate(os.listdir(directory))):
        ...
        if os.path.isfile(os.path.join(directory, file_name)):
            ...
            with open('./save/roi.pkl', 'wb') as file:
...
def clear_temp_folders():
    clear_folder(cropped_line_region_save_path_base)
    clear_folder(cropped_word_regions_save_path_base)
    clear_folder('./save/data/paragraph_wise_predicted_word_box') 
    clear_folder('./save/data/roi')
    clear_folder('./save/data/predicted_text')
    clear_folder('./save/data/word_gc_all_box_all_lines')   
...
def nvidia_deepseek_ocr_correction_api_call(system_prompt,user_prompt ):
    client = OpenAI(
...
def clean_ocr_html(html_content: str) -> str:
    system_prompt = f"""
1. Provide a very short summary about what this is about (In Bengali)
2. Extract natural reading text from this given ocr reconstructed html. Preserve order (css : top position in same column, detect column by left values, some columns maybe part of same column)
3. Correct any ocr errors in the text
4. If text is very little / half cropped, respond with 'ছবিতে পুরো লেখাটা আসেনি, আবার চেষ্টা করুন।'
5. Output only in plain text, no markdown formatting
"""
    user_prompt = f"""
{html_content}
"""
    cleaned_text = qwen_cerbas_ocr_html_extraction_api_call(system_prompt, user_prompt)
    #filename with timestamps
    with open(f'cleaned_text_debug_{int(time.time())}.txt', 'w', encoding='utf-8') as f:
        f.write(cleaned_text)
    parts = cleaned_text.split("</think>", 1)
    new_text = parts[1].lstrip() if len(parts) > 1 else cleaned_text
    return strip_markdown(new_text.strip())
...
# Alternative function using espeak-ng (requires system installation)
def bangla_text(html_file_path: str, output_dir: str = None) -> str:
    import tempfile
    # Read HTML file
    with open(html_file_path, 'r', encoding='utf-8') as f:
        html_content = f.read()
    cleaned_text = clean_ocr_html(html_content)
    # Create temporary text file
    with tempfile.NamedTemporaryFile(delete=False, mode='w', encoding='utf-8', suffix='.txt') as temp_txt_file:
        temp_txt_file.write(cleaned_text)
...
#     # Using gTTS (no system dependencies, generates MP3)
#     audio_path = bangla_html_to_speech(
#         html_file_path="path/to/your/bangla_file.html",
#         output_dir="./output"
#     )
#     print(f"Generated audio: {audio_path}")
    
#     # OR using espeak-ng (requires installation, generates WAV)
#     # audio_path = bangla_html_to_speech_espeak(
#     #     html_file_path="path/to/your/bangla_file.html",
#     #     output_dir="./output"
#     # )
...
if __name__ == "__main__":
    ...
    # reciver queue manager
    manager = QueueManager(address=('', 50000), authkey=b'abcf')
# start manager server in background (non-blocking) so rest of __main__ continues   
    manager.start()
    print("Queue server started (background)")
    ...
    # np ndarray
    #imgs_list = [cv2.imread(img_path) for img_path in imgs_list]
    #print(f"Number of images found: {len(imgs_list)}")
    
    # test_image_directory = 'eval_imgs_paperedge_local_it1'
    img_src_save_dir = f'./recon_batch_{test_image_directory}/html/img'
    cropped_line_region_save_path_base = f"./save/cropped_line_region_{test_image_directory}"
    cropped_word_regions_save_path_base = f"./save/cropped_word_regions_{test_image_directory}"
    ...
    #   ) as pool:
    print("Pools created")
    # read filename from queue (block until available)
    queue = manager.get_queue()
    try:
        while True:
            print("Waiting for new filename on queue...")
            # Get one job dict from the queue (blocks until an item is available)
            job = queue.get()
            print(f"Received job: {job}")
            file_path = job.get('input_file')
            uuid = job.get('uuid')
            public_key = job.get('public_key')

            print(f"Received filename: {file_path}")

            if not os.path.isfile(file_path):
                print("Received path is not a file, skipping.")
                continue

            file_name = Path(file_path).name
            file_stem = Path(file_path).stem

            # per-file output dirs
            img_src_save_dir = f'./save'
            Path(img_src_save_dir).mkdir(parents=True, exist_ok=True)

            cropped_line_region_save_path = os.path.join(cropped_line_region_save_path_base, file_stem)
            cropped_word_regions_save_path = os.path.join(cropped_word_regions_save_path_base, file_stem)
            os.makedirs(cropped_line_region_save_path, exist_ok=True)
            os.makedirs(cropped_word_regions_save_path, exist_ok=True)
            ...
            roi, runtime = run_yolo_model(model_weight, file_path, file_name, img_src_save_dir, standalone_OCR_required=False)

            # persist roi (same as reconstruct did)
            with open('roi.pkl', 'wb') as f:
                pickle.dump(roi, f)

            # generate reconstruction HTML for this single file
            generate_html(roi, file_name)
#
            runtime['html'] = round(time.time() - start_time, 2)
#           
            html_file_path = './save/html/'+file_stem+'.html'
            # convert html to speech

            speech_file = bangla_text(html_file_path)
            finished_job = {
                'uuid': uuid,
            ...
            ## append runtime to CSV log (creates file if not exists)
            out_manager.connect()
            out_queue = out_manager.out_queue()
            out_queue.put(finished_job)
            clear_temp_folders()
            try:
                df_row = pd.DataFrame([{'img': file_name, 'full_pipeline': sum(runtime.values())}])
                write_header = not Path(log_csv_file).exists()
                #df_row.to_csv(log_csv_file, mode='a', header=write_header, index=False)
            except Exception:
                pass
            #print("Finished processing", file_name, "runtime:", runtime)

    except KeyboardInterrupt:
        # release queue manager (cleanup)
        print("Interrupted, exiting.")
    except Exception as e:
        print("Error while processing queue item:", e)
    finally:
        manager.shutdown()
