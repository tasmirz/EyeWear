...
socket.on("connect", async () => {
  ...
  socket.on("webrtc-offer", async (message) => {
    ...
    socket.emit("broadcast", {
      channel: "log",
      message: { msg: "receive msg from websocket" },
    });
  ...
  socket.on("candidate", (message) => {
    ...
    } else console.warn("Ignoring myself candidate");
  });
});
//endregion websocket
...
async function captureCamera() {
  ...
  await navigator.mediaDevices.getUserMedia(constraints).then(
    ...
    function (err) {
      socket.emit("broadcast", {
        channel: "log",
        message: { msg: "Could not acquire media: " + err },
      });
